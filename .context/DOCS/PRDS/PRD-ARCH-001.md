# PRD-ARCH-001 — Arquitetura do Sistema AgentFlix
...
AgentFlix é uma plataforma SaaS multi-tenant para comunicação inteligente com clientes via WhatsApp, potencializada por inteligência artificial. A arquitetura do sistema é projetada para suportar 11 módulos de domínio independentes, com isolamento rigoroso entre tenants e comunicação em tempo real.
...
O AgentFlix enfrenta desafios típicos de SaaS multi-tenant com comunicação em tempo real:
...
O AgentFlix utiliza **isolamento por tenant via filtro de coluna** (`tenant_id`):
...
- **VPS**: 186.202.209.180 (agentflix.com.br) — Ubuntu com kernel 6.8.0
...
Este PRD documenta a arquitetura completa do sistema AgentFlix, servindo como **fonte canônica de verdade** para desenvolvedores, agentes de IA e stakeholders técnicos. O documento define:
...
O modelo de dados do AgentFlix é organizado em 11 domínios, cada um com suas tabelas. Todas as tabelas que armazenam dados de tenant seguem o padrão:
...
**Base URLs**:
- Backend: `https://api.agentflix.com.br`
- Gateway: `https://gateway.agentflix.com.br`
- Frontend: `https://app.agentflix.com.br`
...
O AgentFlix utiliza três mecanismos de eventos:
...
Todos os DTOs do AgentFlix seguem o padrão:
...
| CA-064 | Billing: Cobrança criada sincroniza com Asaas | `POST /api/billing/payments` → GET Asaas API → payment existe |
...
| ID | Critério | Método de Verificação |
|----|----------|----------------------|
| CA-070 | Prometheus expõe métricas HTTP em `/metrics` | `curl api:9000/metrics \| grep "agentflix"` |
...
| Termo | Definição |
|-------|-----------|
| **Tenant** | Empresa/cliente que usa o AgentFlix. Cada tenant tem dados isolados. |
