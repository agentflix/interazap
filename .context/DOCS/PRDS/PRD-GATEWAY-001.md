...
O Gateway do AgentFlix e um servidor WebSocket standalone construido com NestJS 11 e Socket.io, responsavel por manter conexoes persistententes com os clientes frontend (aplicacao Angular) e redistribuir eventos em tempo real originados no backend Laravel e em servicos internos. O Gateway atua como uma camada de bridge entre a API Laravel e os clientes conectados, viabilizando comunicacao bidirecional em tempo real sem que o frontend precise pollingar endpoints REST.

A arquitetura de realtime e parte critic a do produto AgentFlix, que opera como plataforma SaaS multi-tenant para comunicacao inteligente com clientes via WhatsApp, integrando CRM, Chat, Billing e IA. A experiencia do usuario depende fundamentalmente da entrega em tempo real de:
...
O Gateway opera como um servico independente (separate deployment unit) que se comunica com o ecossistema AgentFlix atraves de tres canais principais:
...
4. **Recebimento (Frontend):** O servico `ChatRealtimeService` do frontend Angular (no pacote `@agentflix/chat`) conecta-se ao Gateway via Socket.io, autentica-se com o token JWT e se inscreve nas rooms relevantes (tenant room, ticket rooms). Os eventos chegam como callbacks registrados.
...
O modulo Gateway foi concebido inicialmente como um servico de chat streaming para receber webhooks de provedores de WhatsApp. Com a evolucao do AgentFlix, o Gateway expandiu-se para cobrir:
...
O modulo Gateway e responsavel por prover infraestrutura de comunicacao em tempo real entre o ecossistema AgentFlix e os clientes conectados. O modulo divide-se em duas responsabilidades principais:
...
```javascript
const socket = io('https://gateway.agentflix.com.br/ws', {
  auth: { token: 'eyJ...' },
...
| `DATABASE_URL` | string | `postgres://agentflix:secret@localhost:5432/agentflix` | Connection string PostgreSQL |
...
*Documento gerado em 2026-03-28. Versao 1.0 — rascunho.*
